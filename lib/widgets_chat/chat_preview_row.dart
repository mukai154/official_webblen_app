  ...
  Widget build(BuildContext context) {
          ...
          Row(
            ...
            children: <Widget>[
              ...
              Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: <Widget>[
                  Fonts().textW700(chatName, 24.0, seenByUser ? FlatColors.darkGray : Colors.white, TextAlign.left),
                  SizedBox(height: 4.0),
                  Container(
                    constraints: BoxConstraints(maxWidth: MediaQuery.of(context).size.width - 90),
                    child: lastMessageType == "text"
                        ? Fonts().textW500(
                            lastMessageSentBy + ": " + lastMessageSent, 14.0, seenByUser ? FlatColors.lightAmericanGray : Colors.white, TextAlign.left)
                        : lastMessageType == "image"
                            ? Fonts()
                                .textW500(lastMessageSentBy + ": Sent an Image", 14.0, seenByUser ? FlatColors.lightAmericanGray : Colors.white, TextAlign.left)
              ...
              Padding(
                ...
                child: numberOfUsersInChat > 2
                    ? numberOfUsersInChat == 3
                        ? Fonts().textW300("+ 1 other", 12.0, seenByUser ? FlatColors.darkGray : Colors.white, TextAlign.right)
                        : Fonts().textW300("+${numberOfUsersInChat - 1} others", 12.0, seenByUser ? FlatColors.darkGray : Colors.white, TextAlign.right)
                    : Container(),
              ),
    ...
    final userCard = new Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: seenByUser ? [Colors.white, Colors.white] : [FlatColors.webblenRed, FlatColors.webblenPink]),
      ),
    );

    return GestureDetector(
      onTap: transitionToChat,
      child: Container(margin: EdgeInsets.only(bottom: 1.0), child: userCard),
    );
  }
}
