...
class StreamCommentData extends StatelessWidget {
  ...
  StreamCommentData({this.currentUser, this.postID, this.scrollController});
    ...
    (){
          CommentDataService().deleteComment(comment.commentKey).then((error){
            if (error.isNotEmpty){
              ...
              ShowAlertDialogService().showFailureDialog(context, 'Uh oh...', 'There was an issue deleting this comment.');
            } else {
              Navigator.of(context).pop();
            }
          });
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder(
      ...
      builder: (context, AsyncSnapshot<QuerySnapshot> snapshot) {
        ...
        if (!snapshot.hasData) {
          return Center(child: Fonts().textW300('Loading...', 18.0, FlatColors.lightAmericanGray, TextAlign.center));
        } else {
          if (snapshot.data.documents.isEmpty) return Center(child: Fonts().textW300('This post has no comments', 18.0, FlatColors.lightAmericanGray, TextAlign.center));
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index){
              if (comments[index].contentType == 'start'){
                ...
                return CurrentUserCommentRow(comment: comments[index], deleteAction: () => deleteCommentDialog(context, comments[index]));
              } else {
                return CommentRow(
                  comment: comments[index],
                  onClickAction: () => transitionToUserPage(context, comments[index].uid),
                );
              }
            },
          );
...
class StreamCommentCountData extends StatelessWidget {
  ...
  final String postID;
  StreamCommentCountData({ this.postID});
      ...
      builder: (context, AsyncSnapshot<QuerySnapshot> snapshot) {
        if (!snapshot.hasData) {
          return Padding(
            ...
            child: Fonts().textW500('...', 18.0, FlatColors.lightAmericanGray, TextAlign.center),
          );
        } else {
          return Padding(
            padding: EdgeInsets.only(top: 8.0),
            child: Fonts().textW500('${snapshot.data.documents.length - 1}', 18.0, FlatColors.lightAmericanGray, TextAlign.center),
          );
        }
      },
    );
...
class StreamRequestCommentData extends StatelessWidget {
  ...
  final ScrollController scrollController;
  StreamRequestCommentData({this.currentUser, this.requestID, this.scrollController});
...
class StreamRequestCommentCountData extends StatelessWidget {
  ...
  final String requestID;
  StreamRequestCommentCountData({this.requestID});
      ...
      builder: (context, AsyncSnapshot<QuerySnapshot> snapshot) {
        if (!snapshot.hasData) {
          ...
          return Padding(
            ...
            child: Fonts().textW500('${snapshot.data.documents.length - 1 == -1 ? 0 : snapshot.data.documents.length - 1}', 14.0, FlatColors.lightAmericanGray, TextAlign.right),
          );
        }
      },
    );
