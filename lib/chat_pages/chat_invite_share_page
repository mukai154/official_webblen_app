...
class ChatInviteSharePage extends StatefulWidget {
  ...
  final Event event;
  ChatInviteSharePage({this.currentUser, this.event});
      ...
      if (chatKey == null) {
        ChatDataService().createChat(widget.currentUser.uid, invitedUsers).then((res) {
          if (res != null && res != 'error') {
            ...
            Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => ChatPage(currentUser: widget.currentUser, chatKey: res)));
          } else {
            Navigator.of(context).pop();
          }
        });
        ...
        Navigator.of(context).pop();
        Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => ChatPage(currentUser: widget.currentUser, chatKey: chatKey)));
      }
    });
  }
      ...
      if (chatKey == null) {
        ChatDataService().createChat(widget.currentUser.uid, invitedUsers).then((res) {
          if (res != null && res != 'error') {
            ChatDataService()
                .sendMessage(
                    res, widget.currentUser.username, widget.currentUser.uid, DateTime.now().millisecondsSinceEpoch, widget.event.eventKey, "eventShare")
                .then((error) {
              if (error.isEmpty) {
                Navigator.of(context).pop();
                Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => ChatPage(currentUser: widget.currentUser, chatKey: res)));
                ...
                Navigator.of(context).pop();
                ShowAlertDialogService().showFailureDialog(context, "Uh Oh! 😬", "There was an issue sharing this event. Please Try Again Later.");
              }
            });
          } else {
            Navigator.of(context).pop();
        ...
        ChatDataService()
            .sendMessage(
                chatKey, widget.currentUser.username, widget.currentUser.uid, DateTime.now().millisecondsSinceEpoch, widget.event.eventKey, "eventShare")
            .then((error) {
          if (error.isEmpty) {
            Navigator.of(context).pop();
            Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => ChatPage(currentUser: widget.currentUser, chatKey: chatKey)));
            ...
            GestureDetector(
                ...
                child: Padding(
                  ...
                  child: MediaQuery(
                    ...
                    child: Fonts().textW500(widget.event == null ? "Create" : "Share", 18.0, FlatColors.darkGray, TextAlign.center),
                  ),
                ))),
        body: Container(
