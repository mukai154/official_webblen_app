...
class CommunitiesInAreaPage extends StatefulWidget {
  ...
  CommunitiesInAreaPage({this.currentUser, this.communities, this.action, this.areaName});
  ...
  Widget build(BuildContext context) {
    final appBar = AppBar(
      ...
      title: MediaQuery(
        ...
        child: Fonts().textW700(widget.action == 'newEvent' ? 'New Event' : widget.action == 'newPost' ? 'New Post' : 'My Communities in ${widget.areaName}',
            20.0, Colors.black, TextAlign.center),
      ),
                      ...
                      return CommunityRow(
                        ...
                        onClickAction: widget.action == 'newEvent'
                            ...
                            : widget.action == 'newPost'
                                ? () => PageTransitionService(context: context, currentUser: widget.currentUser, community: activeUserComs[index])
