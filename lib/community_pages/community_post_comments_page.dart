  ...
  void transitionToPostAuthorProfile() async {
    ...
    UserDataService().getUserByName(widget.newsPost.username).then((result) {
      if (result != null) {
        ...
        PageTransitionService(context: context, currentUser: widget.currentUser, webblenUser: result).transitionToUserDetailsPage();
      } else {
        Navigator.of(context).pop();
        ShowAlertDialogService().showFailureDialog(context, "That's Odd...", "There was an issue. Please try again later.");
      }
    });
  }
