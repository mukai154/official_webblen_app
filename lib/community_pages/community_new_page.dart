...
class CreateCommunityPage extends StatefulWidget {
  ...
  CreateCommunityPage({this.areaName});
          ...
          invitedUsers.forEach((uid){
            WebblenNotificationDataService().sendCommunityInviteNotif(currentUser.uid, widget.areaName, newCommunity.name, uid, "@${currentUser.username} invited you to join ${newCommunity.name}").then((error){
              if (error.isEmpty){
                ...
                Navigator.of(context).pop();
                Navigator.of(context).pop();
                ShowAlertDialogService().showFailureDialog(context, "There's good news and bad news...", "You're community was created, but we had issues let your invites know. Please try inviting them again.");
              }
            });
          });
        } else {
          Navigator.of(context).pop();
          ShowAlertDialogService().showFailureDialog(context, 'Uh Oh', 'There was an issue creating your community. Please try again.');
        }
      });
          ...
          child: Form(
            ...
            child: ListView(
              children: <Widget>[
                ...
                Container(
                  padding: EdgeInsets.only(top: 8.0, left: 16.0, right: 8.0, bottom: 4.0),
                  color: Colors.white,
                  child: Row(
                ...
                Container(
                  padding: EdgeInsets.only(top: 8.0, left: 16.0, right: 8.0, bottom: 16.0),
                  color: Colors.white,
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: <Widget>[
