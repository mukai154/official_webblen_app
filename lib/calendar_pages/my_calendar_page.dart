...
import 'package:webblen/widgets_calendar/calendar_event_row.dart';
...
class MyCalendarPage extends StatefulWidget {
  ...
  MyCalendarPage({this.currentUser});
...
class _MyCalendarPageState extends State<MyCalendarPage> with SingleTickerProviderStateMixin {
  ...
  transitionToNewReminderPage(CalendarEvent selectedEvent) async {
    ...
    if (event != null) {
      ...
      DateTime eventDay = DateTime(eventDateTime.year, eventDateTime.month, eventDateTime.day);
      if (allEvents[eventDay] == null) {
        allEvents[eventDay] = [];
      }
      List eventsOnDay = allEvents[eventDay].toList(growable: true);
  ...
  transitionToEventDetailsPage(CalendarEvent event) async {
    ...
    if (event.type == 'reminder') {
      Navigator.of(context).pop();
      PageTransitionService(context: context, currentUser: widget.currentUser, calendarEvent: event, eventIsLive: false).transitionToReminderPage();
    } else {
      Event webblenEvent = await EventDataService().getEventByKey(event.key);
      Navigator.of(context).pop();
      if (webblenEvent != null) {
        PageTransitionService(context: context, currentUser: widget.currentUser, event: webblenEvent, eventIsLive: false).transitionToEventPage();
      } else {
        ShowAlertDialogService().showFailureDialog(context, "Uh Oh", "There was issue loading this event. Please try again later");
      }
    }
  }
      ...
      res.forEach((event) {
        if (filter == 'all' || filter == event.type) {
          ...
          if (DateTime.now().difference(eventDateTime) < Duration(hours: 12)) {
            ...
            CalendarEventDataService().deleteEvent(widget.currentUser.uid, event.key);
          }
        }
      });
      isLoading = false;
      ...
      child: Scaffold(
        appBar: WebblenAppBar().actionAppBar(
          ...
          Row(
            children: <Widget>[
              IconButton(
                icon: Icon(FontAwesomeIcons.slidersH, color: Colors.black, size: 18.0),
                onPressed: () => ShowAlertDialogService().showCalendarFilterDialog(
                    context, filter, () => filterEvents("all"), () => filterEvents("saved"), () => filterEvents("created"), () => filterEvents("reminder")),
              ),
              IconButton(
                icon: Icon(FontAwesomeIcons.edit, color: Colors.black, size: 18.0),
        ...
        body: Container(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              isLoading ? LoadingScreen(context: context) : _buildTableCalendar(),
