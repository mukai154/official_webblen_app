...
class ChatDataService {
  ...
  Future<Stream<QuerySnapshot>> getMessagesFromChat(String chatKey) async {
    Stream<QuerySnapshot> messagesSnapshots = chatRef.document(chatKey).collection('messages').orderBy('dateSent', descending: true).limit(20).snapshots();
    return messagesSnapshots;
  }

  Future<Stream<QuerySnapshot>> getMessagesByUser(String uid) async {
    Stream<QuerySnapshot> messagesSnapshots = chatRef.where('users', arrayContains: uid).orderBy('lastMessageTimeStamp', descending: true).snapshots();
    return messagesSnapshots;
  }
  ...
  Future<String> createChat(String currentUID, List<String> userIDs) async {
    ...
    await Firestore.instance.collection("chats").document(chatDocKey).setData(chat.toMap()).whenComplete(() {
      sendMessage(chatDocKey, '', currentUID, currentDateInMilli, "", "initial");
      result = chatDocKey;
    }).catchError((e) {});
    return result;
  }
  ...
  Future<String> checkIfChatExists(List<String> userIDs) async {
    String chatKey;
    userIDs.sort();
    QuerySnapshot query = await chatRef.where('users', isEqualTo: userIDs).getDocuments();
  ...
  Future<String> sendMessage(String chatKey, String currentUsername, String uid, int timestamp, String content, String messageType) async {
    String error = '';
    DocumentSnapshot chatDoc = await chatRef.document(chatKey).get();
    WebblenChat chat = WebblenChat.fromMap(chatDoc.data);
    if (messageType == 'text') {
      chat.lastMessagePreview = content.length > 20 ? content.substring(0, 20) + "..." : content;
    ...
    WebblenChatMessage newMessage =
        WebblenChatMessage(username: currentUsername, uid: uid, timestamp: timestamp, messageContent: content, messageType: messageType);
    await Firestore.instance
        .collection("chats")
        .document(chatKey)
        .collection('messages')
        .document(timestamp.toString())
        .setData(newMessage.toMap(), merge: true)
        .then((result) async {
