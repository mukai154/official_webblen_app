...
class AdDataService {
  ...
  final CollectionReference adsRef =
      Firestore.instance.collection("native_ads");
  final StorageReference storageReference = FirebaseStorage.instance.ref();

  Future<String> uploadAd(
      File adImage, LocalAd ad, double lat, double lon) async {
    String error = '';
    GeoFirePoint adLoc = geo.point(
      latitude: lat,
      longitude: lon,
    );
    final String adKey = "${Random().nextInt(999999999)}";
    String fileName = "$adKey.jpg";
    ad.imageURL = await FileUploader().upload(
      adImage,
      fileName,
      'native_ads',
    );
