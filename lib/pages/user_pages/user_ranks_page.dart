  ...
  Future<Null> initialize() async {
    BaseAuth().currentUser().then((val) {
      ...
      Firestore.instance
          ...
          .then((userDoc) {
        if (userDoc.exists) {
          StreamUserData.getUserStream(uid, getUser)
    ...
    if (widget.simLocation != null) {
      ...
      UserDataService()
        ...
        currentLon,
          ...
          .then((users) {
        nearbyUsers = users;
        nearbyUsers.sort((userA, userB) =>
            userB.eventHistory.length.compareTo(userA.eventHistory.length));
        isLoading = false;
        setState(() {});
      });
    } else {
      LocationService().getCurrentLocation(context).then((location) {
        if (this.mounted) {
          if (location != null) {
            ...
            UserDataService()
                .getNearbyUsers(currentLat, currentLon)
                .then((users) {
              nearbyUsers = users;
              nearbyUsers.sort((userA, userB) => userB.eventHistory.length
                  .compareTo(userA.eventHistory.length));
              isLoading = false;
              setState(() {});
            });
          }
                  ...
                  child: ListView.builder(
                    itemBuilder: (context, index) {
                      return UserRow(
                        ...
                        isFriendsWithUser: currentUser.friends
                            .contains(nearbyUsers[index].uid),
                        sendUserFriendRequest: () =>
                            sendFriendRequest(nearbyUsers[index]),
                        transitionToUserDetails: () => PageTransitionService(
                          context: context,
                          currentUser: currentUser,
                          webblenUser: nearbyUsers[index],
