...
class _MyCommunitiesPageState extends State<MyCommunitiesPage> {
  ...
  Future<void> getUserCommunities() async {
    ...
    await CommunityDataService().getUserCommunities(widget.uid).then((result) {
      ...
      pendingUserComs
          .sort((comA, comB) => comA.name[1].compareTo(comB.name[1]));
    });
  ...
  List<Community> getCommunitiesInArea(String areaName) {
    return activeUserComs
        .where((com) => com.areaName == areaName)
        .toSet()
        .toList(
          growable: true,
        );
      ...
      title: MediaQuery(
        ...
        child: Fonts().textW700(
          widget.action == 'newEvent'
              ? 'New Event'
              : widget.action == 'newPost' ? 'New Post' : 'My Communities',
          20.0,
          Colors.black,
          TextAlign.center,
        ),
