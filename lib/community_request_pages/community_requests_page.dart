...
import 'package:webblen/widgets_community/community_request_row.dart';
...
class CommunityRequestsPage extends StatefulWidget {
  ...
  final double simLat;
  final double simLon;
  CommunityRequestsPage({this.currentUser, this.areaName, this.simLocation, this.simLat, this.simLon});

  @override
  _CommunityRequestsPageState createState() => new _CommunityRequestsPageState();
}
...
//      recentRequests.sort((reqA, reqB) => reqA.datePostedInMilliseconds.compareTo(reqA.datePostedInMilliseconds));
  ...
  Future<void> voteAction(CommunityRequest req, String voteUpOrDown) async {
    ...
    if (voteUpOrDown == 'up'){
      if (!req.upVotes.contains(widget.currentUser.uid)){
        List upVotes = req.upVotes.toList(growable: true);
        upVotes.add(widget.currentUser.uid);
        req.upVotes = upVotes;
        if (req.downVotes.contains(widget.currentUser.uid)){
          List downVotes = req.downVotes.toList(growable: true);
    ...
    if (didChangeVote){
      setState(() {});
      await CommunityRequestDataService().updateVoting(req.requestID, req.upVotes, req.downVotes);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final appBar = AppBar (
      ...
      title: Fonts().textW700('Community Suggestions', 18.0, Colors.black, TextAlign.center),
      leading: BackButton(color: FlatColors.darkGray),
      actions: <Widget>[
        IconButton(
          icon: Icon(FontAwesomeIcons.plus, color: Colors.black, size: 18.0),
          onPressed: () => PageTransitionService(context: context, currentUser: widget.currentUser, areaName: areaName).transitionToCreateCommunityRequestPage(),
        ),
      ],
    );
    ...
    return Scaffold(
          ...
          body: Container(
            child: isLoading
                ? LoadingScreen(context: context, loadingDescription: 'Loading Suggestions...')
                : LiquidPullToRefresh(
              ...
              child: allRequests.isEmpty
                ...
                children: <Widget>[
                  Column(
                    ...
                    children: <Widget>[
                      ...
                      MediaQuery(
                        data: MediaQuery.of(context).copyWith(textScaleFactor: 1.0),
                        child: Fonts().textW300("There are currently no suggestions", 18.0, FlatColors.lightAmericanGray, TextAlign.center),
                      ),
                      CustomColorButton(
                ...
                itemBuilder: (context, index){
                  return CommunityRequestRow(
                    ...
                    upVoteAction: () => voteAction(allRequests[index], 'up'),
                    downVoteAction: () => voteAction(allRequests[index], 'down'),
                    transitionToComRequestDetails: null,//() => PageTransitionService(context: context, currentUser: widget.currentUser, comRequest: popularRequests[index]).transitionToCommunityRequestDetailsPage(),
