    ...
    bg.BackgroundGeolocation.onGeofence((bg.GeofenceEvent event) async {
      ...
      if (currentDateInMilliseconds - lastNotifTime > 1800000){
        if (timeUntilEventInMilliseconds <= 1800000 && timeUntilEventInMilliseconds > 0){
          if (ranNum == 1){
            CreateNotification().createImmediateNotification("There's an event happening soon nearby", 'You Might Not Want to Miss This One 😶', event.identifier);
          } else if (ranNum == 2){
            CreateNotification().createImmediateNotification("Oh Snap! We Found Something You Might Be Interested in Attending", 'Check It Out Now 🎉', event.identifier);
          } else {
            CreateNotification().createImmediateNotification("$eventTitle will be happening soon!", 'Hope to See You There 🙌', event.identifier);
          }
  ...
  addAndCreateGeoFencesFromEvents(double lat, double lon, String uid){
    ...
    EventDataService().getEventsNearLocation(lat, lon, false).then((events){
      ...
      events.forEach((event){
        String geoIdentifier = event.eventKey + ":" + event.title + "-" + event.startDateInMilliseconds.toString() + "." + event.endDateInMilliseconds.toString();
