...
class _EventDetailsPageState extends State<EventDetailsPage> {
        ...
        Column(
          children: <Widget>[
            ...
            widget.event.startDateInMilliseconds == null
                ? Container()
                : Fonts().textW500('${formatter.format(DateTime.fromMillisecondsSinceEpoch(widget.event.startDateInMilliseconds))}', 18.0, FlatColors.darkGray,
                    TextAlign.left),
          ],
        ),
      ],
  ...
  void editEventAction() {
    ...
    PageTransitionService(context: context, event: widget.event, currentUser: widget.currentUser, isRecurring: false).transitionToCreateEditEventPage();
  }
  ...
  void shareEventAction() {
    Navigator.of(context).pop();
    PageTransitionService(context: context, currentUser: widget.currentUser, event: widget.event).transitionToChatInviteSharePage();
  }
  ...
  void addEventToCalendar() async {
    ...
    CreateNotification()
        .createTimedNotification(randomBetween(0, 99), widget.event.startDateInMilliseconds - 900000, widget.event.title, "Event Starts in 15 Minutes!", '');
    CalendarEvent calEvent = CalendarEvent(
        ...
        timezone: timezone,
        dateTime: formatter.format(DateTime.fromMillisecondsSinceEpoch(widget.event.startDateInMilliseconds)),
        type: 'saved');

    CalendarEventDataService().saveEvent(widget.currentUser.uid, calEvent).then((e) {
      Navigator.of(context).pop();
      if (e.isEmpty) {
        ShowAlertDialogService().showSuccessDialog(context, "Event Saved!", "This Event is Now in Your Calendar");
      } else {
        ShowAlertDialogService().showFailureDialog(context, "Uh Oh!", 'There was an issue saving this event. Please try again later');
      }
    });
  }
  ...
  Widget build(BuildContext context) {
                          ...
                          Material(
                            ...
                            child: Padding(
                                ...
                                child: widget.event.flashEvent
                                    ...
                                    : Fonts()
                                        .textW500('${widget.event.communityAreaName}/${widget.event.communityName}', 14.0, Colors.black, TextAlign.center)),
                          ),
                        ],
                      ),
                      Column(
                          ...
                          widget.event.address == null
                              ...
                              : Container(
                                  ...
                                  child: Fonts().textW400(
                                      '${widget.event.address.replaceAll(', USA', '').replaceAll(', United States', '')}', 16.0, Colors.black, TextAlign.left),
                                ),
                        ],
                      ),
                    ],
                  ...
                  Column(
                    children: <Widget>[
                      ...
                      Container(
                        constraints: BoxConstraints(maxWidth: MediaQuery.of(context).size.width * 0.8),
                        child: Fonts().textW400(
                            formatter.format(DateTime.fromMillisecondsSinceEpoch(widget.event.startDateInMilliseconds)), 16.0, Colors.black, TextAlign.left),
                      ),
                    ],
                  ),
                ],
    ...
    return Scaffold(
      appBar: WebblenAppBar().actionAppBar(
          widget.event.title, IconButton(icon: Icon(FontAwesomeIcons.ellipsisH, size: 18.0, color: Colors.black), onPressed: () => showEventOptions())),
      body: eventView(),
    );
  }
}
...
class RecurringEventDetailsPage extends StatelessWidget {
            ...
            Container(
              ...
              child: event.recurrenceType == 'daily'
                  ...
                  : event.recurrenceType == 'weekly'
                      ...
                      : Fonts().textW400('Every ${event.dayOfTheMonth} ${event.dayOfTheWeek} from  ${event.startTime} to ${event.endTime}', 14.0, Colors.black,
                          TextAlign.start),
            )
          ],
        ),
  ...
  Widget build(BuildContext context) {
    ...
    showEventOptions() {
      ShowAlertDialogService().showEventOptionsDialog(context, null, null, null, deleteEventAction);
    }
        ...
        child: ListView(
          children: <Widget>[
            Container(
              ...
              child: Stack(
                children: <Widget>[CachedNetworkImage(imageUrl: event.imageURL, fit: BoxFit.cover, height: 300.0, width: MediaQuery.of(context).size.width)],
              ),
            ),
            eventCaption(),
            Padding(
                    ...
                    children: <Widget>[
                      ...
                      event.address == null
                          ...
                          : Container(
                              constraints: BoxConstraints(maxWidth: MediaQuery.of(context).size.width * 0.8),
                              child: Fonts()
                                  .textW400('${event.address.replaceAll(', USA', '').replaceAll(', United States', '')}', 14.0, Colors.black, TextAlign.left),
                            ),
                    ],
                  ),
                ],
            ...
            Padding(padding: EdgeInsets.only(left: 16.0, top: 24.0), child: eventDate(context)),
